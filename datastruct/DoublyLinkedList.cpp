...
DoublyLinkedList::DoublyLinkedList(sf::RenderWindow &window, sf::Font &font) : mWindow(window), mFont(font), mType(0), mSmallType(0)
{
    mButton.resize(12);
    mBCreate.resize(2);
    mBInsert.resize(3);
...
DoublyLinkedList::Node::Node(std::string data)
{
    this->data = data;
    this->prev = nullptr;
    this->next = nullptr;
...
void DoublyLinkedList::update(bool mousePress, sf::Vector2i mousePosition, char &keyPress, int &mData, float dt)
{
    for (int i = 0; i < 8; i++)
...
void DoublyLinkedList::updateCreate(bool mousePress, sf::Vector2i mousePosition, char &keyPress)
{
...
void DoublyLinkedList::updateInsert(bool mousePress, sf::Vector2i mousePosition, char &keyPress)
{
    char tempkeyPress;
...
void DoublyLinkedList::updateRemove(bool mousePress, sf::Vector2i mousePosition, char &keyPress)
{
...
void DoublyLinkedList::updateModify(bool mousePress, sf::Vector2i mousePosition, char &keyPress)
{
...
void DoublyLinkedList::updateSearch(bool mousePress, sf::Vector2i mousePosition, char &keyPress)
{
...
int DoublyLinkedList::Rand(int MAX)
{
    srand(time(NULL));
    return rand() % MAX;
}

void DoublyLinkedList::randomize()
{
    std::ofstream outFile("data/randomize.data");
...
void DoublyLinkedList::setPos(std::vector<DataNode> &temp, int id, float start, Node* tmp)
{
    if (tmp == nullptr)
        return;
    if (size == 1)
    {
        temp[0] = DataNode(sf::Vector2f(start + 0 * 100, 150), sf::Vector2f(start + 0 * 100, 150), sf::Vector2f(start + 1 * 100, 150), tmp->data, "head-tail-0", mFont, sf::Color::Black, sf::Color::Black, pallete[color].first, sf::Color::Black, 100.f, tmp->prev != nullptr, tmp->next != nullptr);
        return;
    }
    for (int i = id; i < size; i++)
    {
        temp[i] = DataNode(sf::Vector2f(start + i * 100, 150), sf::Vector2f(start + (i > 0 ? i - 1 : i) * 100, 150), sf::Vector2f(start + (i < size - 1 ? i + 1 : i) * 100, 150), tmp->data, i == 0 ? "head-0" : (i == size - 1 ? "tail-" + std::to_string(i) : std::to_string(i)), mFont, sf::Color::Black, sf::Color::Black, pallete[color].first, sf::Color::Black, 100.f, tmp->prev != nullptr, tmp->next != nullptr);
        tmp = tmp->next;
    }
}

void DoublyLinkedList::create(std::string filename)
{
    ...
    for (int i = 1; i < size; i++)
    {
        ...
        temp.push_back(DataNode(sf::Vector2f(350 + i * 100, 150), sf::Vector2f(350 + (i > 0 ? i - 1 : i) * 100, 150), sf::Vector2f(350 + (i < size - 1 ? i + 1 : i) * 100, 150), tmp->data, i == size - 1 ? "tail-" + std::to_string(i) : std::to_string(i), mFont, sf::Color::White, pallete[color].second, pallete[color].second, pallete[color].second, 0.f, false, false));
        mDataNode.push_back(temp);
        temp[i].mPrev = true;
        mDataNode.push_back(temp);
        tmp->prev = tail;
...
void DoublyLinkedList::insert(int index, std::string element)
{
    if (firstTime == false) return;
    ...
    temp[size] = DataNode(sf::Vector2f(350 + index * 100, 250), sf::Vector2f(350 + index * 100, 250), sf::Vector2f(350 + index * 100, 250), newNode->data, "", mFont, sf::Color::Black, sf::Color::Black, pallete[color].first, sf::Color::Black, 0, false, false);
    mDataNode.push_back(temp);
    ...
    else
    {
        ...
        for (int i = 0; i <= index - 1; i++)
        {
            ...
            temp[i] = DataNode(sf::Vector2f(350 + i * 100, 150), sf::Vector2f(350 + (i > 0 ? i - 1 : i) * 100, 150), sf::Vector2f(350 + (i < size - 1 ? i + 1 : i) * 100, 150), previous->data, i == 0 ? "head-0" : (i == size - 1 ? "tail-" + std::to_string(i) : std::to_string(i)), mFont, sf::Color::White, pallete[color].second, pallete[color].second, sf::Color::Black, 100.f, previous->prev != nullptr, previous->next != nullptr);
            mDataNode.push_back(temp);
        ...
        temp[size].setPosition(sf::Vector2f(350 + index * 100, 250), sf::Vector2f(350 + (index - 1) * 100, 150), sf::Vector2f(450 + index * 100, 150));
        temp[size].mPrev = newNode->prev != nullptr;
        mDataNode.push_back(temp);
        ...
        if (tmp != nullptr) 
        {
            ...
            temp[index].setPosition(sf::Vector2f(450 + index * 100, 150), sf::Vector2f(350 + (index - 1) * 100, 150), sf::Vector2f(450 + (index + 1) * 100, 150));
            mDataNode.push_back(temp);
        }
        ...
        mDataNode.push_back(temp);
        temp[index].setPosition(sf::Vector2f(450 + index * 100, 150), sf::Vector2f(350 + index * 100, 250), sf::Vector2f(450 + (index + 1) * 100, 150));
        mDataNode.push_back(temp);
...
void DoublyLinkedList::modify(int index, std::string element)
{
    if (firstTime == false) return;
...
void DoublyLinkedList::search(std::string element) 
{
...
void DoublyLinkedList::setColor()
{
    for (int i = 0; i < mDataNode.size(); i++)
    {
